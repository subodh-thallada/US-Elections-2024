...
const BettingOddsMap = () => {
    ...
    // Calculate winners based on percentages
    const dataWithWinners = bettingOddsData.map((item) => {
    ...
    locations: dataWithWinners.map((item) => stateAbbreviations[item.state]),
    z: dataWithWinners.map((item) => (item.winner === "Democrat" ? 1 : 0)),
    text: dataWithWinners.map(
        (item) => `
        State: ${item.state}<br>
